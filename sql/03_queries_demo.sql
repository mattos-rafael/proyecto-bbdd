...
SELECT estudiante.nombre, COUNT(calificacion) AS "aprobados", MIN(vertical.vertical) AS "vertical"
FROM calificacion
...
SELECT e.nombre, COUNT(c.resultado) AS "aprobados", MIN(p.promocion) AS "promocion", MIN(e.fecha_comienzo) AS "fecha_comienzo"
...
SELECT t.tipo_proyecto, COUNT(c.resultado) AS "aprobados"
FROM calificacion AS c
...
-- 5 Proyectos con mas aptos
SELECT t.tipo_proyecto, COUNT(c.resultado) AS "suspensos"
FROM calificacion AS c
JOIN tipo_proyecto AS t ON c.tipo_proyecto_id = t.tipo_proyecto_id
...
SELECT v.vertical, COUNT(e) AS "estudiantes"
...
SELECT c.nombre, COUNT(e) AS "num estudiantes"
FROM estudiante AS e
